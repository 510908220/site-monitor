...
def browser_check(url):
    ...
    chrome_script = os.path.join(cur_dir, 'chrome.py')
...
def dig_check(domain):
    ...
    try:
        ...
        if not c.out.strip():
            res = '0'
        tim = '%.2f' % ((time.time() - start_time) * 1000)
        return {'res': res, 'tim': tim, 'sta': '', 'detailed': c.out}
    except Exception as e:
        tim = '%.2f' % ((time.time() - start_time) * 1000)
        return {'res': '0', 'tim': tim, 'sta': str(e), 'detailed': str(e)}


def http_check_ex(url):
    timeout = 10
    result = http_check(url, method='get', timeout=10, is_301=True)
    headers = {
        'User-Agent': 'Mozilla/5.0 (Windows NT 6.1; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/63.0.3239.132 Safari/537.36',
        'Range': 'bytes=0-1040000'
    }
    try:
        r = requests.get(url, timeout=(10, timeout), headers=headers, verify=False)
        detailed = r.text
    except Exception as e:
        detailed = str(e)
    finally:
        result.update({'detailed': detailed})
    return result


def main():
    print(http_check('https://www.google.com'))  # 'Connection timed out
    print(ping_check('212.58.128.20'))
    print(tcp_check('212.58.128.20', 27017))
